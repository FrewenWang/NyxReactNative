...
export class TimeoutManager {
    ...
    public constructor(timerManagerName: string = 'TimerManager') {
        this.name = `${timerManagerName}${++TimeoutManager.timerManagerId}`;
     ...
     */
    public startTimeout(callback: () => void, delay: number, name?: string): number {
        if (this.hasRelease) {
    ...
    private doStartTimeout(
        ...
        let timer = setTimeout((): void => {
            if (callback) {
                console.log(TAG, `Timer: ${this.getTimerName(timerId, timerName)} has processed`);
                callback();
            }
    ...
    public resetTimeout(timerId: number | undefined): void {
        ...
        if (timerMsg) {
            let timer = this.doStartTimeout(timerId, timerMsg.name, timerMsg.callBack, timerMsg.delay);
            this.notifyTimerStart(timerId, timerMsg.name, timer, timerMsg.callBack, timerMsg.delay);
        } else {
            Logger.log(TAG, `could not find Timer:${this.getTimerName(timerId)}`);
        }
    }
    ...
    public clearTimer(timerId: number | undefined): boolean {
        ...
        if (this.timeoutMap.has(timerId)) {
            ...
            if (timerMsg?.timer) {
                ...
                console.log(TAG, `Timer: ${this.getTimerName(timerId, timerMsg.name)} has canceled`);
                return true;
            }
        }
    ...
    public getTimerName(keyTimerID: number | undefined, timerName?: string): string {
        if (timerName) {
    ...
    private notifyTimerStart(
        ...
        console.log(TAG, `startTimer : ${this.getTimerName(keyTimerID, timerName)}, time out delay: ${delay}`);
