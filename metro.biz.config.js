...
const fs = require('fs');
// Module模块拆分映射文件
const platformModuleIDMapFile = require('./toolkits/metro/multi_bundle/platformIdMap.json');
const platformModuleNameMapFile = require('./toolkits/metro/multi_bundle/platformNameMap.json');
...
function postProcessModulesFilter(module) {
    ...
    const projectRootPath = __dirname;
    if (
        (platformModuleIDMapFile == null || platformModuleIDMapFile.length === 0) &&
        (platformModuleNameMapFile == null || platformModuleNameMapFile.length === 0)
    ) {
        console.log('请先打基础包,退出构建');
        process.exit(1);
        return false;
    }
